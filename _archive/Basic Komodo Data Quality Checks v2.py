...
# MAGIC ## Overview
# MAGIC
# MAGIC The framework performs automated data quality checks on Komodo data. To add a new QC check, create a check function below and add it to the `run_all_dq_checks()` function.
# MAGIC
# MAGIC ## Configuration
# MAGIC
...
# MAGIC - **Expected values**: Lists of valid values for categorical fields
...
# MAGIC
# MAGIC Key metrics:
# MAGIC - NULL count for critical fields (medical_event_id, patient_id, service_date, etc.)
# MAGIC - Percentage of NULL values in optional fields (service_to_date, units, unit_type, etc.)
# MAGIC - Logical completeness (e.g., procedure_code_type should be NULL when procedure_code is NULL)
# MAGIC
# MAGIC ### 2. Uniqueness
# MAGIC
# MAGIC Ensures that primary keys are unique within their context. **TO UPDATE as medical_event_id alone is not the primary key**
# MAGIC
# MAGIC Key metrics:
# MAGIC - Duplicate count for medical_event_id values
# MAGIC
# MAGIC ### 3. Validity
# MAGIC
...
# MAGIC
# MAGIC Key metrics:
# MAGIC - Service dates not in the future
# MAGIC - Service dates not unreasonably old (before 1900) **TO UPDATE maybe to 1935?**
# MAGIC - Procedure code types match expected values
# MAGIC - NDC codes conform to expected format (11 digits)
# MAGIC - Units are positive and not extreme
# MAGIC - Unit types match expected values
...
# MAGIC
# MAGIC Key metrics:
# MAGIC - Service date ≤ service_to_date
...
# MAGIC - Distinct procedure code count changes
# MAGIC
# MAGIC ## Architecture
# MAGIC
# MAGIC ### Components
...
# MAGIC 3. Executes the appropriate checks
...
# Define expected unit types (add actual expected values here)
EXPECTED_UNIT_TYPES = ["ML", "UN", "GR", "F2", "ME", "Other"] # Example values

# Define NPI columns to check
NPI_COLS = ["billing_npi", "rendering_npi", "referring_npi"]

# Define thresholds
NULL_PERCENT_THRESHOLD = 5.0  # Max acceptable percentage of NULLs
EXTREME_UNITS_THRESHOLD = 9999
...
def check_completeness(
    ...
    Args:
        ...
        run_id: The unique identifier for this DQ run.
        run_start_time: The timestamp when this DQ run started.
        events_table_name: The specific events table to check (e.g., 'Stg_mx_events').
        current_refresh_month: The month ('yyyy-MM') for which to run the checks.
        db_name: The name of the database/catalog.
        raw_schema: The name of the schema within the database.

    Returns:
        A Spark DataFrame containing the results of the completeness checks,
        conforming to the global `result_schema`, or None if no records are found.
    """
    ...
    try:
        ...
        # Persist for multiple actions if dataset is large enough to benefit
        ...
        # --- Check 1: Mandatory Non-NULL Columns ---
        ...
        for col_name in null_check_cols:
            if col_name not in events_df.columns:
                print(f"Warning: Column '{col_name}' not found in table '{events_table_name}'. Skipping NULL check.")
        ...
        # --- Check 2: Conditional Completeness Check ---
        if "procedure_code" in events_df.columns and "procedure_code_type" in events_df.columns:
            ...
            results.append(create_result_row(spark, run_id, run_start_time, events_table_name, current_refresh_month,
             ...
             print(f"Warning: Skipping Procedure Code Type Consistency check as 'procedure_code' or 'procedure_code_type' not found in {events_table_name}.")


        # --- Check 3: Percentage-Based NULL Checks ---
        percent_null_check_cols = ["service_to_date", "units", "unit_type", "rendering_npi", "referring_npi", "ndc11"]
        for col_name in percent_null_check_cols:
            if col_name not in events_df.columns:
                print(f"Warning: Column '{col_name}' not found in table '{events_table_name}'. Skipping NULL percentage check.")
                results.append(create_result_row(spark, run_id, run_start_time, events_table_name, current_refresh_month,
        ...
        # Ensure unpersist happens even on error if df exists
...
def check_uniqueness(
    ...
    """
    Checks for duplicate primary keys ('medical_event_id') within a specific month
    in the specified events table.

    Args:
        spark: The SparkSession object.
        run_id: The unique identifier for this DQ run.
        run_start_time: The timestamp when this DQ run started.
        events_table_name: The specific events table to check.
        current_refresh_month: The month to check data for, in 'yyyy-MM' format.
        db_name: The name of the database containing the events table.
        raw_schema: The name of the schema containing the events table.

    Returns:
        A Spark DataFrame containing a single row summarizing the uniqueness check results,
    ...
    primary_key_col = "medical_event_id" # Define the primary key column
    ...
    try:
        ...
        # Note: This assumes medical_event_id *should* be unique within the month's data.
        # Adjust logic if duplicates are expected across different data sources within the same table/month.
...
def check_validity(
    ...
    """
    Performs various data validity checks on the specified events table for a specific month
    using a single-pass aggregation for efficiency.
    ...
    Returns:
        A Spark DataFrame containing multiple rows, each summarizing a specific
        validity check result, or None if no records are found or an error occurs.
    """
    ...
    try:
        ...
        # Define columns needed for checks to verify their existence
        required_cols = {
            "service_date", "service_to_date", "procedure_code_type", "ndc11",
            "units", "unit_type"
        }.union(set(NPI_COLS)) # Add NPI columns
        ...
        # Define conditions for each check, guarding against missing columns (already checked above)
        cond_future_service_date = col("service_date") > current_date()
        cond_past_service_date = year(col("service_date")) < 1900 # Consider making 1900 configurable
        ...
        # NDC check: remove hyphens before checking length
        ...
        # Unit type check: ensure EXPECTED_UNIT_TYPES is defined and populated
        cond_invalid_unit_type = col("unit_type").isNotNull() & ~upper(col("unit_type")).isin(EXPECTED_UNIT_TYPES)

        # Build aggregation expressions dynamically for NPI columns
        npi_agg_exprs = []
        for npi_col in NPI_COLS:
            ...
            npi_agg_exprs.append(spark_sum(when(condition, 1).otherwise(0)).alias(alias))

        # Combine all aggregation expressions
        all_agg_exprs = [
            spark_sum(when(cond_future_service_date, 1).otherwise(0)).alias("future_service_date_count"),
            spark_sum(when(cond_past_service_date, 1).otherwise(0)).alias("past_service_date_count"),
            ...
            count("*").alias("total_count") # Use count(*) for total instead of sum(lit(1))
        ] + npi_agg_exprs

        # Execute the single aggregation action
        ...
        # Check if the aggregation result is valid and if there were any rows
        ...
        # Check 1: Service Date not in the Future
        count_val = aggregated_counts["future_service_date_count"]
        status = "PASS" if count_val == 0 else "FAIL"
        detail = f"{count_val} of {total_records} records have service_date in the future."
        results.append(create_result_row(
            spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
            "Service Date Not Future", "Future Date Count", count_val, status, detail
        ))

        # Check 2: Service Date not Ancient
        count_val = aggregated_counts["past_service_date_count"]
        status = "PASS" if count_val == 0 else "WARN" # Warn for potentially old but valid dates
        detail = f"{count_val} of {total_records} records have service_date before 1900."
        results.append(create_result_row(
            spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
            "Service Date Not Ancient", "Ancient Date Count", count_val, status, detail
        ))

        # Check 3: Service To Date not in the Future
        ...
        # Check 4: Procedure Code Type Validity
        ...
        # Check 6: Units Positive
        count_val = aggregated_counts["negative_units_count"]
        status = "PASS" if count_val == 0 else "FAIL" # Usually non-positive units are an error
        ...
        # Check 9: NPI Format (Loop through NPI columns)
        for npi_col in NPI_COLS:
            alias = f"invalid_{npi_col}_format_count"
            ...
            results.append(create_result_row(
                spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
                f"{npi_col} Format (10 digits)", "Invalid Format Count", count_val, status, detail
            ))
...
def check_consistency(
    ...
    """
    Performs various data consistency checks for a specific month using
    optimized joins and aggregations where possible.
    ...
    Args:
        ...
        events_table_name: The specific events table to check.
        current_refresh_month: The month to check data for, in 'yyyy-MM' format.
        db_name: The name of the database containing the relevant tables.
        raw_schema: The name of the schema containing the relevant tables.

    Returns:
        A Spark DataFrame containing multiple rows summarizing consistency checks,
        or None if no event records are found or an error occurs.
    """
    ...
    try:
        ...
        # Retrieve and filter the events table data for the current month
        events_df = (
            get_table(spark, db_name, raw_schema, events_table_name) # Use passed table name
            .filter(date_format(col("kh_refresh_date"), "yyyy-MM") == current_refresh_month)
            .alias("ev")
        )

        # Persist events data if it will be reused multiple times
        events_df.persist(StorageLevel.MEMORY_AND_DISK)

        # Check for empty events data early
        ...
        total_event_records = events_df.count() # Get total count after filtering

        # Get latest demographic records (distinct patients) up to the current month
        ...
        # Get latest provider records (distinct NPIs) up to the current month
        latest_providers = (
        ...
        # Get relevant enrollment periods up to the current month
        enroll_df = (
            ...
            # Add filtering for valid enrollment dates if necessary (e.g., enroll_start <= enroll_end)
            .filter(col("enroll_start").isNotNull() & col("enroll_end").isNotNull())
            .alias("en")
        )

        # Get latest mortality records with death dates up to the current month
        mortality_df = (
        ...
        # Check 1: Service Date Order (Intra-table)
        print(f"Checking Service Date Order for {events_table_name}...")
        if "service_date" in events_df.columns and "service_to_date" in events_df.columns:
            invalid_date_order_count = events_df.filter(
                col("ev.service_to_date").isNotNull() &
                (col("ev.service_date") > col("ev.service_to_date"))
            ).count()
            status = "PASS" if invalid_date_order_count == 0 else "FAIL"
            detail = f"{invalid_date_order_count} of {total_event_records} records have service_date > service_to_date."
            results.append(create_result_row(
                spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
                "Service Date <= Service To Date", "Invalid Order Count", invalid_date_order_count, status, detail
            ))
        else:
            print(f"Warning: Skipping Service Date Order check as 'service_date' or 'service_to_date' not found in {events_table_name}.")


        # Check 2: Referential Integrity - Patient ID (Events vs Demographics)
        print(f"Checking Patient ID consistency for {events_table_name}...")
        if "patient_id" in events_df.columns:
            ...
            status = "PASS" if missing_patient_count == 0 else "FAIL" # Missing patient link is usually an error
            ...
            print(f"Warning: Skipping Patient ID consistency check as 'patient_id' not found in {events_table_name}.")


        # Check 3: Referential Integrity - NPIs (Events vs Providers)
        print(f"Checking NPI consistency for {events_table_name}...")
        npi_cols_in_table = [npi for npi in NPI_COLS if npi in events_df.columns]
        if npi_cols_in_table:
            event_npis = None
            for npi_col in npi_cols_in_table:
                current_npis = events_df.select(col(f"ev.{npi_col}").alias("npi")).filter(col("npi").isNotNull())
                if event_npis is None:
                    event_npis = current_npis
                else:
                    # Use union instead of unionByName if alias is the same ('npi')
                    event_npis = event_npis.union(current_npis)

            if event_npis is not None:
                distinct_event_npis = event_npis.distinct()
                distinct_event_npis.persist(StorageLevel.MEMORY_AND_DISK) # Persist distinct NPIs if large
                ...
                if total_distinct_npis > 0:
                    ...
                    status = "PASS" if missing_npi_count == 0 else "WARN" # Missing provider might be a warning
                    detail = (f"{missing_npi_count} of {total_distinct_npis} distinct NPIs from '{events_table_name}' "
                              f"(month {current_refresh_month}) are missing in the latest providers table.")
                    results.append(create_result_row(
                        spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
                     ...
                     results.append(create_result_row(
                        ...
                        f"No non-NULL NPIs found in '{events_table_name}' for month {current_refresh_month}."
                    ))
            ...
            print(f"Warning: Skipping NPI consistency check as no NPI columns ({', '.join(NPI_COLS)}) found in {events_table_name}.")


        # Check 4: Event Date vs Enrollment Periods
        print(f"Checking Enrollment consistency for {events_table_name}...")
        if "patient_id" in events_df.columns and "service_date" in events_df.columns:
            # Need a unique event identifier if available, otherwise count distinct patients/dates
            # Assuming 'medical_event_id' is the unique key for events
            event_key_col = "medical_event_id"
            if event_key_col not in events_df.columns:
                 print(f"Warning: '{event_key_col}' not found in {events_table_name}. Cannot count distinct events outside enrollment.")
                 # Fallback: count distinct patient-date pairs? Or skip?
                 results.append(create_result_row(
                     spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
                     "Event Date vs Enrollment", "Check Status", "SKIPPED", "WARN",
                     f"Event key '{event_key_col}' not found."
                 ))
            else:
                # Join events with enrollment periods
                events_enroll_join = events_df.select(f"ev.{event_key_col}", "ev.patient_id", "ev.service_date").join(
                    enroll_df, # Consider broadcast(enroll_df) if significantly smaller
                    (col("ev.patient_id") == col("en.patient_id")) &
                    (col("ev.service_date") >= col("en.enroll_start")) &
                    (col("ev.service_date") <= col("en.enroll_end")),
                    "left" # Keep all events, mark those without matching enrollment
                )

                # Count distinct events where no matching enrollment period was found
                ...
                detail = f"{events_outside_enrollment_count} distinct events occurred outside any known enrollment period for the patient."
                results.append(create_result_row(
                    spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
                    "Event Date vs Enrollment", "Events Outside Enrollment Count", events_outside_enrollment_count, status, detail
                ))
        else:
             print(f"Warning: Skipping Enrollment consistency check as 'patient_id' or 'service_date' not found in {events_table_name}.")


        # Check 5: Event Date vs Patient Death Date
        print(f"Checking Mortality consistency for {events_table_name}...")
        if "patient_id" in events_df.columns and "service_date" in events_df.columns:
            # Join events with mortality data and count events occurring after death date
            events_after_death_count = (
                events_df.select("ev.patient_id", "ev.service_date")
                .join(
                ...
                .filter(col("ev.service_date") > col("mo.patient_death_date"))
                .count() # Count rows where event is after death
            )

            status = "PASS" if events_after_death_count == 0 else "FAIL" # Events after death are usually errors
            detail = f"{events_after_death_count} events occurred after the patient's recorded death date."
            results.append(create_result_row(
                spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
                "Event Date vs Death Date", "Events After Death Count", events_after_death_count, status, detail
            ))
        else:
             print(f"Warning: Skipping Mortality consistency check as 'patient_id' or 'service_date' not found in {events_table_name}.")
...
def check_distribution(
    ...
    """
    Analyzes and reports demographic and geographic distributions of medical events
    for a specific table and month.
    ...
    Returns:
        A Spark DataFrame summarizing distributions, or None if errors occur.
    """
    ...
    try:
        # --- 1. Load and Prepare Events Data ---
        # Select only necessary columns: a unique event ID, patient ID, service date
        event_key_col = "medical_event_id" # Assume this is the unique key
        required_event_cols = {event_key_col, "patient_id", "service_date", "kh_refresh_date"}
        ...
        events_df_filtered = (
            events_df
            .filter(date_format(col("kh_refresh_date"), "yyyy-MM") == current_refresh_month)
            .select(event_key_col, "patient_id", "service_date")
            .alias("ev")
        )

        # Persist filtered events if reused
        events_df_filtered.persist(StorageLevel.MEMORY_AND_DISK)

        # Check for empty events data after filtering
        ...
        # --- 2. Demographic Distribution (Gender, Age Group) ---
        print(f"Calculating Demographic Distributions for {events_table_name}...")
        try:
            # Get the latest demographic record for each patient up to the current refresh month
            latest_demo = (
                get_table(spark, db_name, raw_schema, DEMO_TABLE)
                .filter(date_format(col("kh_refresh_date"), "yyyy-MM") <= current_refresh_month)
                .withColumn("rn", expr("row_number() OVER (PARTITION BY patient_id ORDER BY kh_refresh_date DESC)"))
                .filter(col("rn") == 1)
                # Calculate age (approximation using year of birth) - handle potential null yob
                ...
                .withColumn("age_group",
                            ...
                            .when(col("age") >= 65, "65+")
                            .otherwise("Unknown")) # Includes null age
                .select("patient_id", "patient_gender", "age_group")
                .alias("dm")
            )

            # Join events with the latest demographic data
            events_with_demo = events_df_filtered.join(broadcast(latest_demo), "patient_id", "inner")
            events_with_demo.persist(StorageLevel.MEMORY_AND_DISK) # Persist joined data

            total_events_with_demo = events_with_demo.count()

            if total_events_with_demo > 0:
                # Aggregate counts for gender and age group
                ...
                # Calculate and format gender distribution string
                gender_dist_parts = []
                ...
                # Calculate and format age group distribution string
                age_dist_parts = []
            ...
            events_with_demo.unpersist() # Unpersist joined data
        ...
        try:
            # Get geographic data valid for the period up to the current month
            geo_df = (
                get_table(spark, db_name, raw_schema, GEO_TABLE)
                .filter(date_format(col("kh_refresh_date"), "yyyy-MM") <= current_refresh_month)
                .select("patient_id", "valid_from_date", "valid_to_date", "patient_state")
                 # Filter for valid geo periods and non-null states
                .filter(col("valid_from_date").isNotNull() & col("valid_to_date").isNotNull() & col("patient_state").isNotNull())
                .alias("geo")
            )

            # Join events with geographic data based on patient_id AND service_date falling within the geo validity period
            events_with_geo = (
                events_df_filtered.join(
                    geo_df, # Consider broadcast(geo_df) if small
                    (col("ev.patient_id") == col("geo.patient_id")) &
                    (col("ev.service_date") >= col("geo.valid_from_date")) &
                    (col("ev.service_date") <= col("geo.valid_to_date")),
                    "inner" # Only keep events that match a valid geo record for the service date
                )
                # Ensure each event is counted only once per state it occurred in during the month
                .select(f"ev.{event_key_col}", "geo.patient_state")
                .distinct()
            )
            events_with_geo.persist(StorageLevel.MEMORY_AND_DISK) # Persist joined distinct data

            # Calculate state distribution
            state_counts = events_with_geo.groupBy("patient_state").agg(count("*").alias("event_count"))

            total_events_with_geo = events_with_geo.count() # Count distinct event-state pairs
            ...
            if total_events_with_geo > 0:
                ...
                state_dist = (
                    ...
                    .orderBy(col("event_count").desc()) # Order by count first
                    .limit(10)
                    .orderBy(col("percentage").desc()) # Then order by percentage for display
                )

                # Collect results and format into a string
                ...
                results.append(create_result_row(
                    ...
                    f"Based on {total_events_with_geo} distinct event-state occurrences with linked geo data valid at time of service."
                ))
            else:
                results.append(create_result_row(
                    spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
                    "State Distribution (Top 10)", "Distribution String", "N/A", "INFO",
                    "No events could be linked to valid geo data for the service date."
                ))
        ...
        if 'events_df_filtered' in locals() and events_df_filtered.is_cached:
            events_df_filtered.unpersist()
        # Handle potential unpersist errors for other DFs if they exist and are cached
...
def check_temporal(
    ...
    Args:
        ...
        run_start_time: The timestamp when this DQ run started.
        events_table_name: The specific events table to check.
        current_refresh_month: The current month to analyze ('yyyy-MM').
        previous_refresh_month: The previous month for comparison ('yyyy-MM').
        db_name: The name of the database.
        raw_schema: The name of the schema.

    Returns:
        A Spark DataFrame summarizing temporal comparisons, or None if errors occur.
    """
    ...
    try:
        # --- 1. Data Loading ---
        # Load events data for the current month, selecting only needed columns
        current_events = (
            get_table(spark, db_name, raw_schema, events_table_name)
            .filter(date_format(col("kh_refresh_date"), "yyyy-MM") == current_refresh_month)
            .select("patient_id", "ndc11") # Select patient_id and ndc11 if exists
            .alias("curr")
        )
        current_events.persist(StorageLevel.MEMORY_AND_DISK)

        # Load events data for the previous month
        previous_events = (
            get_table(spark, db_name, raw_schema, events_table_name)
        ...
        # Unpersist distinct patient lists
        ...
        returning_patients = overlapping_patients # Count is the same
        ...
        if current_patient_count == 0:
            ...
            return _combine_results(results) # Exit early
        ...
        if previous_patient_count == 0:
            ...
            # Proceed to NDC check if possible
        else:
            ...
            # Calculate and report new patients
            ...
            # Calculate and report returning patients (retention)
        ...
        if "ndc11" in current_events.columns and "ndc11" in previous_events.columns:
            # Calculate distinct patient count per NDC for the current month
            ...
            # Calculate distinct patient count per NDC for the previous month
            ...
            # Compare NDC patient counts using a full outer join
            ndc_comparison = (
                ...
                .withColumn("patient_count_change_pct",
                            when(col("previous_patient_count") == 0, lit(None)) # Indicate new NDC
                            .otherwise(((col("current_patient_count") - col("previous_patient_count")) / col("previous_patient_count")) * 100))
            )

            # Identify significant changes (e.g., >50% change AND current count > 10)
            significant_changes = ndc_comparison.filter(
                (spark_abs(col("patient_count_change_pct")) > 50.0) &
                (col("current_patient_count") > 10) # Add threshold to avoid noise from small counts
            ).orderBy(spark_abs(col("patient_count_change")).desc()).limit(20).collect() # Collect top 20 significant changes

            # Format details for the report
            change_details = []
            ...
            detail = (f"{len(significant_changes)} NDCs had >50% patient count change (min 10 current patients). "
                      f"Top changes: {'; '.join(change_details[:5])}") # Show top 5 details
            results.append(create_result_row(
                spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
                "Patient Count by Drug (NDC11)", "Significant Changes Count", len(significant_changes), status, detail
            ))
        else:
            print(f"Warning: Skipping NDC comparison for {events_table_name} as 'ndc11' column is missing in current or previous data.")
            results.append(create_result_row(
                spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
...
def check_volume(
    ...
    """
    Compares key data volume metrics between the current and previous refresh months
    for a specific events table.
    ...
    Returns:
        A Spark DataFrame summarizing volume comparisons, or None if errors occur.
    """
    ...
    try:
        # Define columns needed for aggregation
        agg_cols = [
            count("*").alias("record_count"),
            countDistinct("patient_id").alias("distinct_patient_count"),
            countDistinct("procedure_code").alias("distinct_procedure_code_count")
        ]
        ...
        current_df = get_table(spark, db_name, raw_schema, events_table_name)
        # Check required columns exist before aggregation
        required_cols = ["kh_refresh_date", "patient_id", "procedure_code"]
        if not all(c in current_df.columns for c in required_cols):
             raise ValueError(f"Missing required columns ({required_cols}) in {events_table_name} for volume check.")
        ...
        # Calculate counts for the previous month
        print(f"Aggregating counts for previous month: {previous_refresh_month} in {events_table_name}")
        previous_df = get_table(spark, db_name, raw_schema, events_table_name) # Reload or assume same schema
        if not all(c in previous_df.columns for c in required_cols):
             raise ValueError(f"Missing required columns ({required_cols}) in {events_table_name} for volume check (previous month).")
    ...
    current_proc_count = current_counts_row["distinct_procedure_code_count"] if current_counts_row else 0
    ...
    previous_proc_count = previous_counts_row["distinct_procedure_code_count"] if previous_counts_row else 0
    ...
    # --- 5. Distinct Procedure Code Count Comparison ---
    print(f"Comparing Distinct Procedure Code Counts for {events_table_name}...")
    results.append(create_result_row(
        spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
        "Distinct Procedure Code Count", "Current Month", current_proc_count, "INFO",
        f"Previous month: {previous_proc_count}"
    ))

    if previous_proc_count > 0:
        proc_change_pct = ((current_proc_count - previous_proc_count) / previous_proc_count) * 100
        # Using PATIENT_COUNT_DEV_THRESHOLD for procedures as well, adjust if needed
        status = "PASS" if abs(proc_change_pct / 100) <= PATIENT_COUNT_DEV_THRESHOLD else "WARN"
        detail = f"Change: {proc_change_pct:+.2f}%. Threshold: +/-{PATIENT_COUNT_DEV_THRESHOLD:.0%}"
        results.append(create_result_row(
            spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
            "Distinct Procedure Code Count Change %", "Percentage Change", proc_change_pct, status, detail # Pass float value
        ))
    else:
        results.append(create_result_row(
            spark, run_id, run_start_time, events_table_name, current_refresh_month, check_category,
            "Distinct Procedure Code Count Change %", "Percentage Change", "N/A", "INFO", "Previous month had 0 procedure codes."
        ))
...
def run_check(
    ...
    **kwargs):
    ...
    Args:
        spark: SparkSession.
        check_func: The specific DQ check function to execute.
        run_id: Unique ID for the entire DQ run.
        run_start_time: Start time of the DQ run.
        result_schema_arg: The schema definition for result rows.
        **kwargs: Arguments to pass to the check function (e.g., table name, month).
                  Must include run_id and run_start_time.
    """
    ...
    try:
        ...
        if result_df is not None and not result_df.isEmpty():
            # Check function should return DF conforming to result_schema_arg
            # No need to add run_id/time here as it's done within check_func -> create_result_row
            save_results(spark, result_df) # save_results now handles schema selection
        ...
        error_data = [(
            ...
            check_name.replace("check_", "").capitalize(), # Infer category from func name
...
def run_all_dq_checks(
    ...
    for table_name in EVENTS_TABLES:
        ...
        if previous_refresh_month:
            ...
            skipped_results = []
            for check_func in temporal_checks:
                 category = check_func.__name__.replace("check_", "").capitalize()
...
def main():
    ...
    # spark = SparkSession.builder.appName("DQ_Checks").getOrCreate() # Uncomment if not in Databricks
    global spark
    ...
    try:
        ...
        # Determine previous month robustly using dateutil if available
        previous_month = None
        try:
            ...
            # Basic fallback (less reliable, especially across year boundaries)
            try:
                current_dt = datetime.datetime.strptime(current_month + "-01", "%Y-%m-%d")
                # Go back roughly 30 days and format - adjust logic as needed
                prev_dt = current_dt - datetime.timedelta(days=30)
                previous_month = prev_dt.strftime("%Y-%m")
                print(f"Calculated previous month using basic subtraction: {previous_month}")
                 ...
                 previous_month = None # Ensure it's None if calculation fails
        ...
        if previous_month:
            try:
                ...
                prev_month_exists = (
                    ...
                    .limit(1) # Optimization: just need to know if at least one exists
                    .count() > 0
                )
...
#     # This assumes 'spark' is available globally, typical in Databricks/notebooks
#     # If running as a script, initialize SparkSession here:
